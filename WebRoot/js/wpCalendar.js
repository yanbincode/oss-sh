...
// 一、<input name=txt><input type=button value="Show Calendar"
// onclick="showCalendar(this,document.all.txt)">
...
// 控件外任意点击一点即可关闭该控件
/*
 * 以下为WalkingPoison的修改说明 WalkingPoison联系方式：walkingpoison@hotmail.com
 * 
 * Ver 2.59 修改日期：2008-6-17 1.*修改对Firefox的兼容性，使其可兼容多浏览器（注意：在FF下运行需要将包含js文件的代码放在<body>中）。此版本尚未完善，完善后升级为2.60版
 * 2.存在问题：样式在两个浏览器还不一致；
 * 
 * Ver 2.55 修改日期：2007-6-26 1.年份的下拉框增加了可选项，用于决定年份的滚动是通过点击还是mouseover来触发
 * 
 * Ver 2.54 修改日期：2006-4-30
 * 1.*将日历单元格的宽度和高度改为变量定义，可以自由定义日历的大小（注意：较大的单元格会使日历响应速度变慢） 2.删除了无用的SELECT样式定义
 * 
 * Ver 2.53 修改日期：2005-11-25
 * 1.增加了拖动出iframe的处理，使得拖动的时候更加不容易停下来（但是在出iframe的同时也出了IE窗口的情况会使得日历移动到左上角）
 * 2.增加了日历中翻年月按钮的双击处理，使得点击的响应更快速 3.在日历对象内使用了self变量来访问自身，而不用依赖于外部创建的wpCalendar实例
 * 
 * Ver 2.52 修改日期：2004-10-9 1.优化了writeCalendar函数及其它的部分代码（增加了抽象的日期对象）
 * 2.显示的提示改为与输出格式完全一致
 * 
 * Ver 2.51 修改日期：2004-9-23, 2004-9-29
 ...
 * 
 * Ver 2.5 修改日期：2004-7-23 1.调整了属性和方法的名称 2.增加了themename属性，使得使用代码来修改主题更为方便
 * 
 * Ver 2.5 beta 4 修改日期：2004-4-16, 2004-5-18
 * 1.*增加了调用的方法，最后一个参数允许是一段代码，用于在点击日历以后可以执行需要的代码。 2.修改了一点注释的笔误以及改进了一点代码效率
 * 
 * Ver 2.5 beta 3 1.增加主题可以设置的参数，并在日历版本信息中同时显示主题的版本信息
 * 2.将日历的一些方法集成在日历对象内部，以免与外部函数定义形成冲突 3.增加了DiabloII主题，希望各位喜欢^^ 4.优化部分代码
 * 
 * Ver 2.5 beta 2 1.增加鼠标停止在日期上的渐变效果。需要ie5.5以上支持。 2.增加参数设置对以上效果进行自动判断和测试。
 * 3.解决方法一选择日期后产生的焦点小问题。
 * 
 * Ver 2.5 beta 修改日期：2003-10-27 1.*修改日历的初始化方法，可以对多种参数进行设置。
 * 2.*引入主题的概念，便于用户选择或者自由定制主题。 3.*控件已经内置了几个主题，用户可根据需要进行选用。
 * 
 * Ver 2.1+ 1.继续修正根据目标在窗口的位置自动调整位置，调整了在窗口右方时的显示问题。
 * 
 * Ver 2.1 1.修正选择月份后下一月的提示内容错误 2.日历根据输出目标在窗口的位置自动调整显示位置。()
 * 3.*自定义输入输出格式，并修改使得输入和输出的格式统一。 4.增加自定义的样式，可以使用"M"和"d"，并且日可以空缺。
 * 
 * Ver 2.0 修改日期：2002-12-13 1.*全新修改使用iframe作为日历的载体，不再被select和flash等控件挡住。
 * 2.修正了移植到iframe后移动日历控件的问题。
 * 
 * Ver 1.5 修改日期：2002-12-4 1.选中的日期显示为凹下去的样式 2.修改了关闭层的方法，使得失去焦点的时候能够关闭日历。
 * 3.修改按键处理，使得Tab切换焦点的时候可以关闭控件 4.*可以自定义日历是否可以拖动
 * 
 * Ver 1.4 修改日期：2002-12-3 1.修正选中年/月份下拉框后按Esc键导致年/月份不显示的问题
 * 2.修正使用下拉框选择月份造成的日期错误（字符串转化为数字的问题）
 * 3.*外观样式的改进，使得控件从丑小鸭变成了美丽的天鹅，从灰姑娘变成了高贵的公主，从……（读者可以自己进行恰当的比喻）
 * 4.再次增大年/月份的点击空间，并对下拉框的位置稍作调整
 * 
 * Ver 1.3 修改日期：2002-11-29 1.*空白部分用灰色显示上个月的最后几天和下个月的前几天 2.*每个日期上面加上鼠标提示
 * 3.修改使得当前日期和当前选择的日期的背景色在灰色日期部分也能正常显示
 * 
 * Ver 1.2 修改日期：2002-11-28 1.*修改年和月的点击都把中文包含在内，增大点击的空间 2.当前选择的日期在列表中显示不同的背景色
 * 3.修正了点击单元格之间的分隔线导致控件关闭的问题
 * 
 * Ver 1.1 修改日期：2002-11-15 1.修正了方法二按Esc键关闭以后再次点击不会显示日历的问题
 * 2.点击today直接选中当前的日期并关闭控件 3.*如果调用控件的输入框含有合法日期，则自动显示输入框的日期部分。
...
function wpCalendar(){
	...
	this.MonthinMM	= this.datestyle.indexOf("MM")>=0?true:false;		// added
																		// in
																		// Ver
																		// 2.1
	...
	this.selectToday=function (){	// Today Button
		...
		if(this.month<10 && this.MonthinMM)this.month="0" + this.month;	// added
																		// in
																		// Ver
																		// 2.1
...
function showCalendar() // 主调函数
{
	...
	if(document.body.clientHeight>211 + thei && document.body.clientHeight-(ttop-document.body.scrollTop)<211+thei && ttop-document.body.scrollTop>211)	// added
																																						// in
																																						// Ver
																																						// 2.1
		dads.top = ttop-211
	...
	if(document.body.clientWidth-(tleft-document.body.scrollLeft)<cwidth && tleft-document.body.scrollLeft>cwidth)	// added
																													// in
																													// Ver
																													// 2.2
	...
	// 根据当前输入框的日期显示日历的年月
	var reg = new RegExp("^" + wpCalendar.datestyle.replace((wpCalendar.Dateindd?"dd":"d"),"(\\d{1,2})").replace("yyyy","(\\d{4})").replace((wpCalendar.MonthinMM?"MM":"M"), "(\\d{1,2})")+"$");	// added
																																																	// in
																																																	// Ver
																																																	// 2.1
	...
	if(r!=null){
		var t=getIndex();
		r[t[2]]=r[t[2]]-1; 
		if(wpCalendar.datestyle.indexOf("d")<0)r[t[3]]=1;			// 使用一个"d"就可以包括两种情况
																	// //added
																	// in Ver
																	// 2.1
